...
const Fees = () => {
  ...
  const fetchAll = async () => {
    try {
      setLoading(true);
      setError('');
      const [structRes, accRes, studRes] = await Promise.all([
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      flash(`Payment of LKR ${Number(paymentForm.amount).toLocaleString()} recorded for ${selectedAccount.studentName}.`);
  ...
  const structureCols = [
    ...
    { header: 'Type', cell: (r) => <span className="px-2 py-0.5 text-xs rounded-full bg-indigo-100 text-indigo-800">{r.feeType}</span> },
    { header: 'Grade', cell: (r) => r.gradeLevel ? `Grade ${r.gradeLevel}` : 'All' },
    { header: 'Year', accessor: 'academicYear' },
    { header: 'Term', cell: (r) => r.term || '—' },
    { header: 'Amount', cell: (r) => <span className="font-semibold">LKR {Number(r.amount).toLocaleString()}</span> },
    { header: 'Due Date', cell: (r) => r.dueDate || '—' },
    {
      header: 'Actions',
      cell: (r) => (
        <div className="flex space-x-2">
          <button onClick={() => openEditStructure(r)} className="text-indigo-600 hover:text-indigo-900 text-sm font-medium">Edit</button>
          <button onClick={() => deleteStructure(r)} className="text-red-600 hover:text-red-900 text-sm font-medium">Delete</button>
        </div>
      ),
    },
  ];
  ...
  const accountCols = [
    ...
    { header: 'Fee', cell: (r) => r.feeStructure?.name || '—' },
    { header: 'Total', cell: (r) => `LKR ${Number(r.totalAmount || 0).toLocaleString()}` },
    ...
    { header: 'Balance', cell: (r) => <span className="text-red-600 font-semibold">LKR {Number(r.balanceAmount || 0).toLocaleString()}</span> },
    {
      ...
      cell: (r) => {
        const s = r.status;
        if (s === 'PAID') return <span className="px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-800">Paid</span>;
        if (s === 'PARTIALLY_PAID') return <span className="px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-800">Partial</span>;
        if (s === 'CANCELLED') return <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-500">Cancelled</span>;
      ...
      cell: (r) => (
        <div className="flex space-x-2">
          ...
          <button
            ...
            className="text-red-600 hover:text-red-900 text-sm font-medium disabled:text-gray-400 disabled:cursor-not-allowed"
          >
            Cancel
          </button>
        </div>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage fee structures, student accounts, and payments</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-gray-900">Fee Structures</h3>
